...
from extensions import MoneyException, MoneyConverter
...
def convert(message: telebot.types.Message):
    try:
        ...
        if len(values) != 3:
            raise MoneyException('Параметров должно быть три.')

        quote, base, amount = values
        total_base = MoneyConverter.convert(base, quote, amount)
